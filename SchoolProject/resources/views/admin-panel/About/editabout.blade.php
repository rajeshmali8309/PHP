      ...
      <div class="header">
        ...
        <div class="table-container">            
            ...
            <form action="{{ route('editabout-store') }}" method="post" enctype="multipart/form-data">
                @csrf
                <h2 class="frm">Edit About</h2>

                <input type="hidden" name="id" value="{{ $aboutID->id }}" placeholder="@error('title'){{ $message }} @enderror" id="title" required>
                
                <label for="title">Title:</label>
                <input type="text" name="title" value="{{ $aboutID->title }}" placeholder="@error('title'){{ $message }} @enderror" id="title">
                @error('title')
                   <div class="error">{{ $message }}</div>
                @enderror <br><br>

                <label for="message">About information:</label>
                <textarea name="information" placeholder="Enter About Information" id="message" rows="5">{{ $aboutID->information }}</textarea>
                @error('information')
                   <div class="error">{{ $message }}</div>
                @enderror  <br>
